...
def run_SD_mn(mnList, seqPath, outName):
    ...
    if not os.path.exists("final_decomposition.tsv"):
        ...
        sys_call([pathToSD, seqPath, pathToMon, "-t", "30"])
    os.chdir(origin_dir)
...
def get_blocks_interseq(path_seq, tsva, tsvb):
    ...
    return blocks_inter, bl2cnt, bl1cnt
...
def calc_mean_for_subseq(seq, tsv_B_res, tsv_Ivan_res, CAmn, IAmn, f):
    blocks = get_blocks_interseq(seq, tsv_B_res, tsv_Ivan_res)[0]
...
def main():
    ...
    fa = open(os.path.join(args.outdir, "..", "MonomerComparasion.csv"), "a")
    ...
    Sharedblocks, IAblocks, CAblocks = get_blocks_interseq(args.seq, tsv_MN_res, tsv_Ivan_res)

    fa.write(str(len(MonomersNew_)) + "\t")
    fa.write(str(IAblocks) + "/" + str(CAblocks) + "/" + str(len(Sharedblocks)) + "\t")
